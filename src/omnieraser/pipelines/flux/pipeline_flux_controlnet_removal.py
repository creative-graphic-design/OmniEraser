...
class FluxControlNetInpaintingPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def _unpack_latents(latents, height, width, vae_scale_factor):
        batch_size, num_patches, channels = latents.shape  # 1,2304,64
        height = height // vae_scale_factor  # 768//8=96
    ...
    def __call__(
        ...
        if isinstance(self.controlnet, FluxControlNetModel):
            control_image, height, width = (
                self.prepare_image_with_mask(  # control_image:1, 2304, 68
                    image=control_image,
                    mask=control_mask,
                    width=width,
                    height=height,
        ...
        latents, latent_image_ids = (
            self.prepare_latents(  # latents:1, 2304, 64,     latent_image_ids:1, 2304, 3
                batch_size * num_images_per_prompt,
                num_channels_latents,
                height,
                width,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print('b', text_ids.shape, latent_image_ids.shape)
                # controlnet
                (
        ...
        if output_type == "latent":
            ...
            # print('latents', latents.shape, 'height', height, 'width', width)
